...
c*                   Spline programs                             *
c*****************************************************************
      subroutine iac1r(x,nx,nr4,fx,lx,mxa,fxa,mxb,fxb,tx,cx)
cSm030221
         implicit double precision(a-h,o-z)

         integer lx,mxa,mxb,nx,nr4
         double precision cx,fx,fxa,fxb,tx,x
cSm011006
c         dimension cx(nr4),fx(nx),tx(nr4),x(nx)
       ...
       if (mxa .lt. k3) go to 10
          ...
          go to 20
   10  continue
       call iac1r3(tx,nx,nr4,cx,lx,mxa,mxb,a,b)
   20  continue
       do 30 i = k1, nx
          j = i + k2
          cx(j) = fx(i)
   30  continue
       if (mxa .gt. k2) go to 40
          call iac1r4(tx,nx,nr4,cx,lx,mxa,fxa,mxb,fxb,a,b)
   40  continue
       i = nx + k2
       do 50 j = k1, i
          k = j + k1
          cx(j) = iac1r1(tx,cx,nr4,lx,k)
   50  continue
       return
      end
      double precision function iac1r1(tx,cx,nr4,lx,jx)
cSm030221
         implicit double precision(a-h,o-z)
         integer jx,lx,nr4
...
c         dimension cx(*),tx(*)
   ...
   20  continue
      ...
      subroutine iac1r2(x,nx,nr4,mx,tx)
cSm030221
         implicit double precision(a-h,o-z)
         integer mx,nx,nr4
         double precision tx,x
cSm011006
c         dimension tx(nr4),x(nr4)
         dimension tx(*),x(*)
   ...
   30  continue
      ...
      subroutine iac1r3(tx,nx,nr4,cx,lx,mxa,mxb,ac,bc)
cSm030221
         implicit double precision(a-h,o-z)
         integer lx,mxa,mxb,nx,nr4
         double precision ac,bc,cx,tx
cSm011006
c        dimension ac(3),bc(3),cx(nr4),tx(nr4)
   ...
   10  continue
       ...
       b = c1
       cx(k1) = c1
       a = ias1r2(tx,cx,nr4,k4,mxa,tx(k3),k3,k1)
       if (lx .eq. k0) go to 30
          b = iac1r1(tx,cx,nr4,lx,k2)
          cx(k1) = c0
          cx(k2) = c1
          c = iac1r1(tx,cx,nr4,lx,k3)
          if (mxa .gt. k0) go to 20
             d = ias1r2(tx,cx,nr4,k4,k0,tx(k4),k4,k1)
             cx(k2) = 0
             ac(k2) = c1/c/d
             go to 30
   20  continue
       e = ias1r2(tx,cx,nr4,k4,k0,tx(k3),k3,k1)
       f = ias1r2(tx,cx,nr4,k4,mxa,tx(k3),k3,k1)
       cx(k2) = c0
       cx(k1) = c1
       g = ias1r2(tx,cx,nr4,k4,k0,tx(k3),k3,k1)
       d = g*f - e*a
       ac(k3) = g/d/c
       ac(k2) = a/d/c
       a = g
   30  continue
       ...
       ac(k1) = c1/a/b
       cx(j) = c1
       a = ias1r2(tx,cx,nr4,k4,mxb,tx(j),i,k1)
       if (lx .eq. k0) go to 50
          cx(j) = c0
          cx(l) = c1
          b = iac1r1(tx,cx,nr4,lx,k)
          cx(l) = c0
          cx(k) = c1
          c = iac1r1(tx,cx,nr4,lx,j)
          cx(k) = c0
          cx(i) = c1
          if (mxb .gt. k0) go to 40
             d = ias1r2(tx,cx,nr4,k4,mxb,tx(i),i,k1)
             cx(i) = c0
             bc(k2) = c1/c/d
             go to 50
   40  continue
       e = ias1r2(tx,cx,nr4,k4,k0,tx(j),i,k1)
       f = ias1r2(tx,cx,nr4,k4,mxb,tx(j),i,k1)
       cx(i) = c0
       cx(j) = c1
       g = ias1r2(tx,cx,nr4,k4,k0,tx(j),i,k1)
       d = g*f - e*a
       bc(k3) = g/d/c
       bc(k2) = a/d/c
       a = g
   50  continue
      ...
      subroutine iac1r4(tx,nx,nr4,cx,lx,mxa,fxa,mxb,fxb,ac,bc)
cSm030221
         implicit double precision(a-h,o-z)
         integer lx,mxa,mxb,nx,nr4
...
c         dimension ac(3),bc(3),cx(nr4),tx(nr4)
       ...
       do 10 i = k1, k4
          j = i + k1
          a(i) = iac1r1(tx,cx,nr4,lx,j)
   10  continue
       if (m .gt. k0 .or. lx .eq. k0) go to 30
          c = ias1r2(tx,a,nr4,k4,k0,tx(k4),k3,k0)
          cx(k2) = (cx(k4) - c)*ac(k2)
   20  continue
       a(k1) = iac1r1(tx,cx,nr4,lx,k2)
       a(k2) = iac1r1(tx,cx,nr4,lx,k3)
   30  continue
       c = ias1r2(tx,a,nr4,k4,m,tx(k3),k3,k0)
       if (m .gt. k0) go to 40
          j = k2 - lx
          cx(j) = (cx(k3) - c)*ac(k1)
          go to 60
   40  continue
       if (lx .eq. k1) go to 50
          ...
          go to 60
   50  continue
       b = ias1r2(tx,a,nr4,k4,k0,tx(k3),k3,k0)
   ...
   70  continue
       if (m .gt. k0 .or. lx .eq. k0) go to 90
          c = ias1r2(tx,a,nr4,k4,k0,tx(l),l,k0)
          cx(j) = (cx(l) - c)*bc(k2)
   80  continue
       a(k3) = iac1r1(tx,cx,nr4,lx,k)
       a(k4) = iac1r1(tx,cx,nr4,lx,j)
   90  continue
       c = ias1r2(tx,a,nr4,k4,m,tx(k),l,k0)
  ...
  100  continue
       if (lx .eq. k1) go to 110
          ...
          go to 120
  110  continue
       b = ias1r2(tx,a,nr4,k4,k0,tx(k),l,k0)
  ...
  120  continue
      ...
      double precision function ias1r(tx,nx,nr4,cx,idx,xx)
cSm030221
         implicit double precision(a-h,o-z)
         integer idx,nx,nr4
         double precision cx,tx,xx
cSm011006
c         dimension cx(nr4),tx(nr4)
         dimension cx(*),tx(*)
         external ias1r1,ias1r2
         integer i,ias1r1,k1,k4
         double precision ias1r2
         data k1/1/,k4/4/
       i = ias1r1(tx,nx,nr4,xx)
       ias1r = ias1r2(tx,cx,nr4,k4,idx,xx,i,k1)
       return
      end
      integer function ias1r1(tx,nx,nr4,xx)
cSm030221
         implicit double precision(a-h,o-z)
         integer nx,nr4
...
c         dimension tx(nr4)
   ...
   30  continue
      ...
      double precision function ias1r2(tx,cx,nr4,kx,idx,xx,ix,jx)
cSm030221
         implicit double precision(a-h,o-z)
         integer idx,ix,jx,kx,nr4
         double precision cx,tx,xx
cSm011006
c         dimension cx(nr4),tx(nr4)
         dimension cx(*),tx(*)
...
c      dimensions (nxa,nya) >(nx,ny)
     ...
     1                 nxa,nr4a)
c-----------------------------------------------------------
c      The additional input parameters nxa,nr4a
...
c      nx4x is a maximal value of nr4: nr4a.ge.nr4
c----------------------------------------------------------               
	implicit none
         integer nxa,nr4a
         integer lx,ly,mxa,mxb,mya,myb,ncx,ncy,nfx,nfy,nx,ny,nry
...
c         dimension cxy(ncx,ncy),cy(nry),fxa(ny),fxb(ny),fxy(nfx,nfy),
c     1             fya(nx),fyb(nx),tx(ncx),ty(ncy),x(nx),y(ny)
         dimension cxy(nr4a,*),cy(*),fxa(*),fxb(*),fxy(nxa,*),
     1             fya(*),fyb(*),tx(*),ty(*),x(*),y(*)
  ...
  210  continue
     ...
     1 ncx,ncy,idx,idy,xx,yy,nr4a)
c-----------------------------------------------------------
c      The additional input parameters nr4a
c      are to use this subroutine when the arrays dimensions
