...
cSm050309
     ...
     &pwcpl_t_ar(nnktormax),pwcpl_p_ar(nnkpolmax)
      ...
      double precision rma,zma,psimag,powtotlh,pi,tet,psi0,z,r,
...
c-----external for test
      ...
      if (ilaunch.eq.1) then
         write(*,*)
         write(*,*)'grill_lh: ilaunch=1. All rays start from one point'
         write(*,*)
      endif
...
c     &                     k,wtheta(k),wtheta_edge(k),fpwth(k)
c              enddo
c              write(*,*)'!2 wtheta_edge(nthini+1)',wtheta_edge(nthini+1)
...
c--------------------------------------------------------------------
      write(*,*)'nphiteta cnpar1',cnpar1,'cnteta',cnteta,'cnphi',cnphi
c--------------------------------------------------------------------
      cs=dcos(phi)
      sn=dsin(phi)
cSAP090513
...
c-----input
      double precision theta ! poloidal angle(radians) measured from the
c                              midplane in clockwise direction 
      double precision cnpar !N_parallel
      double precision phi   !toroidal angle (radians)
...
c-----output
      ...
      double precision z_ini,r_ini
      double precision cntheta_ini !N poloidal in rho_ini point
      double precision cnphi_ini   !N_poloidal in rho_ini point 
      integer i_rho_ini_LHFW_found
c-----external zr_psith
      double precision psi_rho,b
c     psi_rho(rho),zr_psith(psi,theta,z,r))
c
c-----local

cSm061107
       double precision psi,rho_loc
...
cSm050408   
      if(i_n_poloidal.eq.1) then
        ...
        call cninit12(z,r,phi,cnpar,cnteta,cnphi,cnz,cnr,cm,iraystop)
      else    
...
cSm070804 cnteta=(cnz*bz+cnr*br)/bmod
        ...
        cnphi=(cm/r)
        write(*,*)'in rho_ini_LHFW after cninit cnteta,cnphi,iraystop',
     &  cnteta,cnphi,iraystop
      endif
