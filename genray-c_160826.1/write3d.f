...
c-----initialization of the number of the ray 
      irayl=0
      nrayl=nray
      write(*,*)'in sub write3d1 nrayl=',nrayl
      return
      end
...
c-----the output ray data for mnemonic.nc netCDF file
      ...
      do is=1,nrayelt
         ...
         !--------------------
         wx_nc(is,irayl)=wx(is)
         wy_nc(is,irayl)=wy(is)
         wz_nc(is,irayl)=wz(is)
         !--------------------         
         wr_nc(is,irayl)=wr(is) !=dsqrt(wx(is)**2 +wy(is)**2)
         wphi_nc(is,irayl)=wphi(is) ! remove later
         wnpar_nc(is,irayl)=wnpar(is)
         wnper_nc(is,irayl)=wnper(is)
         delpwr_nc(is,irayl)=delpwr(is)
         sdpwr_nc(is,irayl)=sdpwr(is)
         do i=1,nbulk  !2,nbulk  ! YuP[2016] why not including e ?
            salphas_nc(is,irayl,i)=real(salphas(is,i)) ! single prec
            wvthermal_nc(is,irayl,i)=real(wvthermal(is,i)) ! single prec
         enddo
...
c         salphal_nc(is,irayl)=salphal(is)
         ...
         !--------------------         
         sb_x_nc(is,irayl)=sb_x(is)
         sb_y_nc(is,irayl)=sb_y(is)
         sb_z_nc(is,irayl)=sb_z(is)
         sb_r_nc(is,irayl)=sb_r(is) 
         sb_phi_nc(is,irayl)=sb_phi(is) ! remove later
         !--------------------         
         wn_x_nc(is,irayl)=wn_x(is)
         wn_y_nc(is,irayl)=wn_y(is)
         wn_z_nc(is,irayl)=wn_z(is)
         wn_r_nc(is,irayl)=wn_r(is) 
         wn_phi_nc(is,irayl)=wn_phi(is) ! remove later
         !--------------------         
         vgr_x_nc(is,irayl)=wvgr_x(is)
         vgr_y_nc(is,irayl)=wvgr_y(is)
         vgr_z_nc(is,irayl)=wvgr_z(is)
         vgr_r_nc(is,irayl)=wvgr_r(is) 
         vgr_phi_nc(is,irayl)=wvgr_phi(is) ! remove later
c--------dielectric tensor
