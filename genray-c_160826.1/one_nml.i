...
c-----from namelist /dispers/
     ...
     &            coll_mult,refl_loss, rho_larm_max, 
...
c-----from namelist /denprof/
     &            dense0,denseb,rn1de,
     &            rn2de, 
     &            zbegin_den_dropoff, zlength_den_dropoff,
...
c-----from namelist /tokamak/  
     +            model_b,
     &            indexrho,ipsi,ionetwo,ieffic, ncoils,
     &            n_wall,max_limiters,n_limiter,  
c-----from namelist /wave/
...
c-----from namelist /tokamak/ 
     ...
     &            deltripl,psifactr, bz0, rbphi0,
     +            rs_frc, 
     +            curc(ncoilsa),radc(ncoilsa),zcoil(ncoilsa),
     +            wall_rmin,wall_rmax,wall_zmin,wall_zmax,
     +            rlim_wall_fraction,     
     +            rmirror,zbox_mirror,rbox_mirror,b00_mirror,
...
c-----namelist /plasma/
     ...
     +            elx0,ely0,elz0, elax,elay,elaz,
     +            dens0rr,dens0es,dens0ub,eltheta,sintt,costt,
     +            Rm0rr,Rm0es,rtau,r_ub_edge,     
c-----namelist /species/
c-----from namelist /varden/
     &            var0,denn,denm,an,sigman,
c-----from namelist /denprof/
     ...
     &            rn2de(nbulka),
     &            zbegin_den_dropoff(nbulka),
     +            zlength_den_dropoff(nbulka),
c-----from namelist /tpopprof/ 
...
c     dqdrho
c     dbmdx,dbmdy,dbmdz,dbmdr,dbmdph -derivatives of the module of magnetic field
