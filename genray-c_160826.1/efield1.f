...
c      cnperp      double complex N_perp                              !
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - !
c								    !
c        output parameters					    !
c     ex,ey,ez				      			    !
...
c      write(*,*)'reps',reps
      ...
      dlamb(1,2)=-reps(1,2)
      dlamb(1,3)=-ccnz*cnperp-reps(1,3)
      
      dlamb(2,1)=-reps(2,1)
      dlamb(2,2)=ccnz*ccnz+cnperp*cnperp-reps(2,2)
      dlamb(2,3)=-reps(2,3)
      
      dlamb(3,1)=-ccnz*cnperp-reps(3,1)
      dlamb(3,2)=-reps(3,2)
      dlamb(3,3)=cnperp*cnperp-reps(3,3)
c      do i=1,3
c        do j=1,3
...
c      write(*,*)'i0=',i0,'j0=',j0
      ...
      cex=aa(i0,1)
      cey=aa(i0,2)
      cez=aa(i0,3) ! here: Ez is along B-field line ?
...
c       if (dreal(cez).eq.0.d0) goto 10
        if(abs(cez).gt.0.d0)then !YuP[11-2016] added this check/condition
...
c      write(*,*)'efield1 cex',cex 
c      write(*,*)'efield1 cey',cey
c      cez1=dsqrt(1.d0-cdabs(cex)**2-abs(cey)**2)
      ex=abs(cex)
      ey=abs(cey)
      ez=abs(cez)
