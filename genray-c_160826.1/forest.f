...
c-----lambda= 0.5 k_perp^2 * rho_gyro^2
      ...
      if(abs(tpop*nps*beta**2).ne.0.d0)then
...
c     +           nps,beta*nll,X,Y,lambda, qmin,qmax,n
      ...
      call ibessn(lambda,n+1,ri01,xne) ! In/exp
...
c            Ibesspp(q+n+1)= xne(2+iabs(q)) 
c     .                    +(2.D0*Iabs(q)+ 1)*xne(iabs(q)+1 )/lambda
c     .                    +(q*q-Iabs(q))*xne(iabs(q))/lambda**2 !not used?
         endif
         ...
         Kxx=  Kxx + X*q*q * Ibess(q+n+1) * A(q+n+1) *one_lambda
         Kxy=  Kxy -i*q*X *(Ibess(q+n+1)-Ibessp(q+n+1))*A(q+n+1)
         Kyy=  Kyy + X*( q*q*Ibess(q+n+1) *one_lambda +
     ...
     +              + ( A(q+n+1)-1.d0 )*vflow/c
         ...
         Kxz= Kxz + X*np*q*Ibess(q+n+1) * B(q+n+1) *one_lambda /  Yc
...
c-----derivatives from N_parallel    
     ...
     .       nll/bmod*dbmdr     
      dnlldphi=(cnz*dbzdph+cnr*dbrdph+cm*dbphdph/r)/bmod-nll/bmod*dbmdph
...
c     for the hot plasma root calculations. 
      ...
      write(*,*)'in hotnp ibw,iraystop=',ibw,iraystop
      i = (0.0d0,1.0d0)
...
c                 boundary of cold of plasma interval
     ...
     .                   vflowc,nllc,cnper_0,1,K))
            write(*,*)'hotnp n_perp_l==cnper_0  D(cnper_0)=',d_left 
            if (dabs(d_left).lt.1.d-7) then
               nperp=cnper_0
               goto 30 !->root is found (got lucky)
            endif
...
c                 boundary of cold of plasma interval
            ...
            d_right=dreal(dhot_sum(nbulkc,massc,xc,yc,tec,tpopc,
     .                   vflowc,nllc,cnper_0,1,K))
            write(*,*)'hotnp n_perp_r==cnper_0  D(cnper_0)=',d_right 
            if (dabs(d_right).lt.1.d-7) then
               nperp=cnper_0
               goto 30 !->root is found (got lucky)
            endif
...
c         write(*,*)'hotnp d_left',d_left 
     ...
     .                      vflowc,nllc,0.d0,1,K)) 
            ...
            d_right=dreal(dhot_sum(nbulkc,massc,xc,yc,tec,tpopc,
     .                    vflowc,nllc,nperp,1,K))
            !write(*,*)'nperp,d_left,d_right', nperp,d_left,d_right
            !pause
...
c-----output      
      ...
      integer iraystop
       ...
       if(ixyz.eq.1) stop 'hotnperp should not be called for ixyz=1'
      cnpar2=nll*nll
...
c-----externals
      double precision x,y,tempe,tpoprho,vflowrho,rhopsi,psif
...
c-----find magnetic axis (psi=0, theta=0)
 ...
 10   continue
      ...
      write(*,*)'forest.f  rho_loc= ',rho_loc
