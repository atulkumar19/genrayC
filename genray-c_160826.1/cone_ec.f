...
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - !
c      	input parameters	(given by genray.in)				    !
...
c               (erg/sec)				            !
c--------------------------------------------------------------------
      subroutine cone_ec(alpha1,na1,na2,alpha2,phist,phin,tetan,powtot,
     1                   nray,alphaj,betaj,powj) !->out
...
c     n_r=  stetan*cphin ! Not used
c     n_phi=stetan*sphin ! Not used
c     n_z=ctetan         ! Not used
c----------------------------------------------------------------
...
c-----------------------------------------------------------
     ...
     1              -cn_eksi*ctetan*cphin -cn_eph*sphin

              cn_phi=cn_en*stetan*sphin
     1              -cn_eksi*ctetan*sphin +cn_eph*cphin
...
c-----------------------------------------------------------------
      ...
      write(*,*)'cone_ec p,powtot,psum',p,powtot,psum
      do iray=1,nray
...
c------------------------------------------------------------------
      ...
      subroutine raypat(thetac, phic, div, cr, nray, gzone, mray,
     &  theta, phi) !->out
cProlog
...
c-----locals
      ...
      real*8 x(n+1),xc(n-1),ar_integral(n+1),total_integral
...
c      stop 'cone_ec disk'
      ...
      do iray_l=1,nray
        ...
        xstj(iray_l)=rstj(iray_l)*dcos(phistj(iray_l))
        ystj(iray_l)=rstj(iray_l)*dsin(phistj(iray_l))
